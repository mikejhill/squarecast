...
describe("board generator", () => {
  ...
  it("reports missing title, insufficient answers, and duplicate content", () => {
    const editor = BoardModel.createDefaultEditor();
    editor.config.title = " ";
    editor.answers = [
    ...
    expect(result.warnings).toContain(
      "Duplicate answer text will appear as separate squares.",
    );
  });
  ...
  it("rejects more constrained answers than available squares", () => {
    const editor = BoardModel.createDefaultEditor();
    editor.config.size = 3;
    ...
    expect(generator.validate(editor).errors).toContain(
      "Only 8 constrained answers can fit on this board.",
    );
  });
  ...
  it("warns when a valid board creates an unusually long URL", () => {
    ...
    editor.answers = editor.answers.map((answer, index) => ({
      ...answer,
      text: `${index}-${"long answer ".repeat(400)}`,
    }));
