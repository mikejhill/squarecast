...
/**
 * Owns the curated, read-only sample library and creates fresh editor states
 * from it. Definitions contain no runtime IDs, so opening the same sample twice
 * still produces independent URL state.
 */
export class SampleBoardCatalog {
  private static readonly definitions: readonly SampleBoardDefinition[] = [
    {
  ...
  public createAllEditors(): EditorState[] {
    return SampleBoardCatalog.definitions.map((definition) =>
  ...
  public createRandomEditor(random: () => number = Math.random): EditorState {
    const index = Math.floor(random() * SampleBoardCatalog.definitions.length);
    // Math.random's [0, 1) contract guarantees an in-range index, and the
    // compile-time catalog is intentionally non-empty.
    const definition = SampleBoardCatalog.definitions[index]!;
